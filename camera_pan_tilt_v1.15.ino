...
float posTilt = 0;
float posPan = 0;
float pan_offset = 12; //verdiene er etter de på kartet, negativ er venstre
float tilt_offset = -6; //negative verdier vil få den til å gå gå mot positive verdier
...
int period = 90; //oppdateringsfrekvensen på servoen
...
void loop() {
  ...
  if (innkommende[0] != 'h' && innkommende[0] != 'a' && innkommende[0] != 'p' && innkommende[0] != 'b' && innkommende[0] != 'f') {  //må ikke være noen av disse verdiene
    ...
    if (posTilt < 8.0/-10.0) posTilt = 8.0/-10.0;           //må endres i samsvar med kontroller (-10.0 (.0 for float) må endres etter hva man deler på ved Axis-), forhindrer feildata!
    else if (posTilt > 8.0/10.0) posTilt = 8.0/10.0;
    
    float posPan = AxisPan.toFloat() / -10;
    if (posPan < 8.0/-10.0) posPan = 8.0/-10.0;
    else if (posPan > 8.0/10.0) posPan = 8.0/10.0;
